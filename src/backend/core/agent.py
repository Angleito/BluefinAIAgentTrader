...
class MockNetworks:
    ...
    def __contains__(cls, item):
        return item in ["MAINNET", "TESTNET", "SUI_STAGING", "SUI_PROD"]
    
    @classmethod
    def keys(cls):
        return ["MAINNET", "TESTNET", "SUI_STAGING", "SUI_PROD"]
    ...
    def __getitem__(cls, key):
        if key == "MAINNET":
            ...
            return cls.TESTNET
        elif key == "SUI_STAGING":
            return cls.SUI_STAGING
        elif key == "SUI_PROD":
            return cls.SUI_PROD
        else:
...
class MockOrderSignatureRequest:
    ...
    def __init__(self, symbol, side, size, price=None, order_type="MARKET", **kwargs):
        self.symbol = symbol
        self.side = side
        self.size = size
    ...
    try:
        ...
        if BLUEFIN_CLIENT_SUI_AVAILABLE and BluefinClient is not None:
            ...
            if private_key and private_key != "your_private_key_here":
                try:
                    # Initialize client with required parameters according to the documentation
                    logger.info(f"Initializing Bluefin client with {network} network")
                    
                    # Handle different network formats
                    network_value = None
                    if hasattr(Networks, network):
                        network_value = getattr(Networks, network)
                    elif network in Networks:
                        network_value = Networks[network]
                    else:
                        logger.warning(f"Network {network} not found in available networks")
                        logger.warning(f"Available networks: {dir(Networks) if hasattr(Networks, '__dict__') else list(Networks.keys()) if hasattr(Networks, 'keys') else 'Unknown'}")
                        logger.warning("Falling back to SUI_PROD")
                        network = "SUI_PROD"
                        if hasattr(Networks, network):
                            network_value = getattr(Networks, network)
                        elif network in Networks:
                    ...
                    if network_value is not None:
                        client = BluefinClient(
                            ...
                            network_value,  # Network configuration
                            private_key  # Wallet private key
                        )
                        logger.info(f"Bluefin client created for {network} network")
                        
                        # The client will be initialized asynchronously in init_clients
                        logger.info("Client created, will be initialized in init_clients function")
                    else:
                        logger.error(f"Could not determine network value for {network}")
                        logger.warning("Falling back to mock client")
                except Exception as e:
                    logger.error(f"Error creating Bluefin client: {e}")
                    logger.error(traceback.format_exc())
                    logger.warning("Falling back to mock client")
            else:
                logger.warning("BLUEFIN_PRIVATE_KEY not found in environment variables or is set to default value")
                logger.warning("Falling back to mock client")
        else:
            logger.warning("Bluefin client not available")
            logger.warning("Falling back to mock client")
    except Exception as e:
...
def init_claude_client():
    ...
    try:
        ...
        primary_model = os.environ.get("CLAUDE_MODEL", "claude-3.7-sonnet")
        fallback_model = os.environ.get("CLAUDE_FALLBACK_MODEL", "claude-3-haiku-20240307")
        
        # Initialize Claude client with API key
        logger.info(f"Initializing Claude client with Anthropic API key")
        ...
        try:
            ...
            # Try a simple message to verify the model works
            ...
            logger.info(f"Primary model {primary_model} is working")
            # Set the model to use for future requests
            os.environ["CLAUDE_ACTIVE_MODEL"] = primary_model
        except Exception as e:
            ...
            try:
                ...
                logger.info(f"Fallback model {fallback_model} is working")
                # Set the model to use for future requests
                os.environ["CLAUDE_ACTIVE_MODEL"] = fallback_model
            except Exception as e:
                logger.error(f"Fallback model {fallback_model} also failed: {e}")
                logger.error("No working Claude models available")
                return None
...
async def init_clients():
    ...
    # Define global clients for the whole application
    global client, claude_client, perplexity_client, perplexity_client
    ...
    if not isinstance(client, MockBluefinClient) and not MOCK_TRADING:
        try:
            ...
            await client.init(True)  # onboard user if first time
            ...
            if account_data:
                ...
                # Log available markets and their leverage
                market_data = account_data.get("accountDataByMarket", [])
                logger.info(f"Available markets:")
                for market in market_data:
                    symbol = market.get("symbol", "Unknown")
                    leverage_wei = market.get("selectedLeverage", "0")
                    leverage = int(leverage_wei) / 10**18
                    logger.info(f"  {symbol}: {leverage:.1f}x leverage")
            else:
    ...
    try:
        ...
        if perplexity_client and perplexity_client.api_key:
            ...
            logger.warning("Perplexity client initialized but API key is missing")
    except Exception as e:
        logger.error(f"Error initializing Perplexity client: {e}")
        logger.error(traceback.format_exc())
        logger.warning("Perplexity client will not be available")
        perplexity_client = None
...
async def analyze_chart_with_claude(screenshot_path, ticker):
    ...
    try:
        ...
        # Use active model if set during initialization
        active_model = os.environ.get("CLAUDE_ACTIVE_MODEL")
        if active_model:
            model = active_model
            logger.info(f"Using active Claude model: {model}")
...
async def execute_trade(
    ...
    if position_size is None and os.getenv("DEFAULT_POSITION_SIZE_PCT"):
        position_size = float(os.getenv("DEFAULT_POSITION_SIZE_PCT", 1))
        
    if risk_percentage is None and os.getenv("DEFAULT_RISK_PCT"):
        risk_percentage = float(os.getenv("DEFAULT_RISK_PCT", 1))
        
    if stop_loss_percentage is None and os.getenv("DEFAULT_STOP_LOSS_PCT"):
    ...
    if leverage is None and os.getenv("DEFAULT_LEVERAGE"):
        leverage = int(os.getenv("DEFAULT_LEVERAGE", 12))
...
async def get_trades(limit: int = 10):
    ...
    try:
        # Get trades from the Bluefin client
        trades = await bluefin_client.get_user_trades_history(limit=limit)
        return trades
    except Exception as e:
...
async def start_api_server():
    # Start the server
    port = int(os.getenv("SOCKET_PORT", "5008"))  # Use 5008 as default from .env
    config = uvicorn.Config(app, host="0.0.0.0", port=port)
    server = uvicorn.Server(config)
    logger.info(f"API server starting on port {port} (from SOCKET_PORT environment variable)")
    try:
        await server.serve()
    except Exception as e:
